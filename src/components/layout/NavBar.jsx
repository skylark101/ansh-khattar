...
export default function NavBar() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="text-2xl font-bold bg-linear-to-r from-primary via-primary/75 to-primary/40 bg-clip-text text-transparent hover:opacity-80 transition-opacity"
              aria-label="home"
            >
      ...
      >
        <div className="bg-black/95 backdrop-blur-lg border-t border-white/10 px-5 py-6 space-y-4">
          {NAV_LINKS.map((link) => (
            <button
              key={link.id}
              onClick={() => handleNavClick(link.id)}
              className={`block w-full text-left px-4 py-3 rounded-lg font0-medium transition-all duration-300 ${
                activeSection
