...
const ProjectCard = ({ project }) => {
  const { title, description, image, technologies, tags, demoUrl, githubUrl } =
    project;
  return (
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {technologies.map((tech, index) => (
            <span key={index} className="px-3 py-1 text-xs font-medium text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transitio-colors duration-200">
              {tech}
            </span>
          ))}
        </div>
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag, index) => (
                <span
                  key={index}
                  className="px-3 py-1 text-sm rounded-full font-medium bg-white/10 text-white/80 border border-white/10"
                >
                  {tag}
                </span>
              ))}
